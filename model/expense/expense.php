            ...
            if ($expenseid != null && is_csrf_valid()) {
                ...
                try {
                    ...
                    $PushData = [
                        ...
                        "expense_date" => $_POST['expense_date'],
                        "expense_added_by" => $_POST['login_id'],
                        "expense_token" => uniqid(),
                        "cash_flow_type" => $_POST['cash_flow_type']
                    ];
                    if ($db->insert("expense_details", $PushData)) {
                        js::alert("Expense Copied Successfully with new date $PushData[expense_date]");
                        js::redirect("/Expense/Manage");
                    } else {
                        error_loger($db->conn->error, __FILE__, "Cant able to Process the request for Copying the Expense ", $_POST['login_id']);
                        throw new Exception("Process Terminated with Error to Copy the Expense");
                    }
                } catch (Exception $e) {
                    js::alert($e->getMessage());
                    js::redirect('/Expense/Manage');
